...
type Prefs = {
  provider: 'cc-r2' | 'gemini' | 'openai' | 'anthropic' | 'openrouter' | 'custom';
  model: string;
  safeMode: boolean;
...
// CC R2 Provider - uses default API key if empty
const isCC2RProvider = (provider: Prefs['provider']) => provider === 'cc-r2';
const getEffectiveProvider = (provider: Prefs['provider'], hasApiKey: boolean) => {
  if (isCC2RProvider(provider)) return 'gemini';
  return provider;
};
...
function App() {
  const [provider, setProviderState] = useState<Prefs['provider']>(() => (localStorage.getItem('ccc-provider') as Prefs['provider']) || 'cc-r2');
  const [model, setModelState] = useState(() => localStorage.getItem('ccc-model') || 'gemini-2.0-flash-exp');
  ...
  const [apiKey, setApiKeyState] = useState(() => localStorage.getItem('ccc-api-key') || '');
  ...
  const setApiKey = (value: string) => { setApiKeyState(value); localStorage.setItem('ccc-api-key', value); };
  return (
...
function CommandCenter() {
  ...
  const [chatReply, setChatReply] = useState<{ reply: string; code: string | null; commandId: string | null; provider: string; model: string; isCC2R?: boolean } | null>(null);
  ...
  const isUsingCC2R = provider === 'cc-r2';
  const runAssistant = () => {
    if (!message.trim()) return;
    // For CC R2, no API key is needed
    if (!isUsingCC2R && !apiKey) { setToast('Add an AI provider key in Settings before sending.'); return; }
    // For CC R2, send empty API key so server uses default
    const effectiveApiKey = isUsingCC2R ? '' : apiKey;
    const effectiveProvider = isUsingCC2R ? 'gemini' : provider;
    assistant.mutate({ data: { sessionId: sessionId || null, message: message.trim(), provider: effectiveProvider, apiKey: effectiveApiKey, model: isUsingCC2R ? 'gemini-2.0-flash-exp' : model, execute: safeMode ? false : true } }, { onSuccess: (response) => { setChatReply(response); if (response.code) setCode(response.code); setMessage(''); qc.invalidateQueries({ queryKey: getGetRuntimeEventsQueryKey(eventParams) }); }, onError: (err) => { 
      // Handle rate limit error for CC R2
      if (err?.response?.status === 429 && isUsingCC2R) {
        setToast('Rate limit reached. CC R2 will auto-recover in 1-2 minutes.');
      } else {
        setToast('The assistant could not complete that request.');
      }
    }});
  };
  const executeCode = () => {
